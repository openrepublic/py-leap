...
void testcontract::checkexasset(const extended_asset& eass, name contract, int64_t checkam) {
    const asset& ass = eass.quantity;
    check(eass.contract == contract,    "extended asset contract name mismatch");
    check(ass.is_amount_within_range(), "magnitude of asset amount must be less than 2^62" );
    check(ass.symbol.is_valid(),        "invalid symbol name" );
    check(ass.amount == checkam,
          "invalid amount: " + std::to_string(ass.amount) + " " +
...
void testcontract::testversion(const int64_t val) {
    print(VERSION);
    print("\n");
    print(val);
}
